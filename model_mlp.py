...
#Documentação do algoritmo https://scikit-learn.org/stable/modules/generated/sklearn.neural_network.MLPClassifier.html
...
#Cada iteração uma imagem é adicionada
...
for i in range(len(onlyfiles)):
    ...
    y = pd.DataFrame(df.pop('class')) # remove a coluna de classes do y
    X = pd.DataFrame(df) #o resto das colunas fica no x    
    ...
    for i, (train_index, test_index) in enumerate(kf.split(X)):
        ...
        mlp = MLPClassifier(hidden_layer_sizes=(15,15,15), activation='relu', solver='adam', max_iter=500) 
        
        mlp.fit(X_train, y_train) #Treina modelo    
        y_pred=mlp.predict(X_test) #Predict do modelo      
